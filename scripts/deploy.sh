...
aws cloudformation create-stack --stack-name falcon-ci-lab-stack --template-url https://$bucketName.s3.amazonaws.com/entry.yaml --parameters ParameterKey=S3Bucket,ParameterValue=$bucketName ParameterKey=RemoteIp,ParameterValue=$trustedIp ParameterKey=RepositoryName,ParameterValue=$repoName ParameterKey=RepositoryUrl,ParameterValue=$repoUrl --capabilities CAPABILITY_NAMED_IAM CAPABILITY_IAM CAPABILITY_AUTO_EXPAND --region $region
else
aws s3api create-bucket --bucket $bucketName --region $region --create-bucket-configuration LocationConstraint=$region
aws s3 cp templates/ s3://$bucketName --recursive
echo -e "$LB\n"
echo -e "Standing up environment$NC"
aws cloudformation create-stack --stack-name falcon-ci-lab-stack --template-url https://$bucketName.s3-$region.amazonaws.com/entry.yaml --parameters ParameterKey=S3Bucket,ParameterValue=$bucketName ParameterKey=RemoteIp,ParameterValue=$trustedIp ParameterKey=RepositoryName,ParameterValue=$repoName ParameterKey=RepositoryUrl,ParameterValue=$repoUrl --capabilities CAPABILITY_NAMED_IAM CAPABILITY_IAM CAPABILITY_AUTO_EXPAND --region $region
fi
